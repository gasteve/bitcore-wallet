...
if (exeName === 'wallet') {
  ...
  program
		...
		/* new wallet commands (non HD) */
    .command('utxos', '[non-HD] fetch wallet utxos')
    .parse(process.argv);
  ...
  var splitName = exeName.split('-');
  if (splitName[0] === 'wallet') {
    require('./'+splitName[1]);
  } else {
    console.log('error: unexpected executable name format');
  }
}
